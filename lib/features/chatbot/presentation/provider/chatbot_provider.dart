...
class ChatMessages extends _$ChatMessages {
  ...
  /// Re-fetches conversation history from the server and rebuilds the
  /// message list. FAQs are already in memory so this is fast.
  /// Called by pull-to-refresh and the refresh banner tap.
  Future<void> refresh() async {
    final locale = ref.read(localeProvider);
    final l10n = _l10nFor(locale);
    try {
      final history =
      await ref.read(chatbotRepositoryProvider).getHistory();
      state = [];
      for (final msg in history) {
        addMessage(msg);
      }
      ...
      debugPrint('[Chatbot] Refresh failed: $e');
    }
  }
